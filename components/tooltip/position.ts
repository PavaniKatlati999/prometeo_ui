...
import getDirection from './getDirections'
...
function getArrowStyles(target, tip, direction, state, props) {
  ...
  // Explicitly type these objects
  const borderStyles: React.CSSProperties = {}; // Correct type
  const positionStyles: React.CSSProperties = {}; // Correct type
  ...
  switch (direction) {
    ...
    default:
      ...
      if (props.background) {
        borderStyles.borderBottom = `8px solid ${props.background}`;
      } else {
...
export default function positions(direction, forceDirection, tip, target, state, props) {
  ...
  if (!forceDirection && tip) {
    ...
    realDirection = getDirection(trimmedDirection, tip, target, props, bodyPadding, testArrowStyles,false)
  }
